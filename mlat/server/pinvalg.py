...
def tdoa_pinv_solve(measurements, debug=None, altitude=10000, altitude_error=None):
    '''
    分组求解
    :param measurements:
    :param altitude:
    ...
    pseudorange_data = [[receiver.position if not debug else receiver,  # 实际传入的参数是类对象，需要receiver.positon
                         (timestamp - base_timestamp) * 1e6 if not debug else (timestamp - base_timestamp),  # s -> us
...
if __name__ == "__main__":
    ...
    mea = [[(-2640995.0, 4321663.0, 3863794.0), 0.0, 0.3], [(-2656287.0, 4317997.0, 3857399.0), 4.0, 0.25],
           [(-2651352.0, 4322976.0, 3855260.0), 21.0, 0.31], [(-2642368.0, 4327651.0, 3856180.0), 31.0, 0.35]]
